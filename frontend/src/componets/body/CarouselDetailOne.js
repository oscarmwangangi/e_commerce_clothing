...
function CarouselDetailOne() {
  const { id } = useParams(); // Get the id from the URL
  const [product, setProduct] = useState(null);

  useEffect(() => {
    // Fetch product data based on the product id
    fetch(`http://localhost:8000/api/carouselOne/${id}/`) // Adjust the URL as needed
  ...
  >
    ...
    {/* <h1 className='text-6xl font-semibold font-sans pt-8 uppercase '>Other items</h1> */}
    <RecentViewed />
    </div>
...
export default CarouselDetailOne;
